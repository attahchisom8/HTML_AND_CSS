...
const performOperation = (expr, oprArr) => {
  ...
  for (let k = 0; k < oprArr.length; k++) {
    ...
    if (item.precedence === 1) {
      const subExpr = expr.substr(item.idx.start_idx + 1, item.idx.end_idx - 1);
      console.log("subExpr: ", subExpr);
      res = eval_simple_expr(subExpr);
    } else {
...
/**
 * eval_simple_expr - evaluates simole mathematical expression
 * @expr: the mathematical expredsion
 *
...
export const eval_simple_expr = (expr) => {
  ...
  let oprArr = extractOperands(expr);
  oprArr = bodmasParser(oprArr);
  res = performOperation(expr, oprArr);

  return res;
  
}

let expr = "-2-8-5+44+3+300/5";
// expr = "(2 - 5) + 5";
// expr = "-2 - 2 / 5 * 4";
const res = eval_simple_expr(expr);
console.log(res);
